...
k_array = np.zeros(len(marray), dtype=complex)
...
positionfunc = gauss_packet(o, draw2[0], 0.1, 0.1)
k_array[i] = fourier_analysis(positionfunc, marray[i], o)
for i in range(0, len(marray)):
    k_array[i] = fourier_analysis(positionfunc, marray[i], o)
...
def animate(i):
    ...
    if i<500:
       ...
       psi_exp = PIR_Func(state, o)*PIR_Time(state, I, i/1000)
        ...
        for j in range(0, len(marray)):
            psi_exp = psi_exp + k_array[j]*PIR_Func(marray[j],o)*PIR_Time(marray[j], I, (i-1000)/10000)
